module.exports = {
  title: "注册",
  username: "用户名",
  email: "邮箱",
  send_email_verify_code: "发送",
  email_verify_code: "邮箱验证码",
  password: "密码",
  retype_password: "确认密码",
  register: "注册",
  empty_username: "用户名不能为空",
  invalid_username: "用户名无效，有效的用户名需要：\n* 仅包含大小写字母、数字和「-_.#$」\n* 长度在 3 到 24 之间",
  username_already_taken: "用户名已被使用",
  empty_email: "邮箱不能为空",
  invalid_email: "邮箱无效",
  email_already_used: "邮箱已被使用",
  empty_password: "密码不能为空",
  invalid_password: "密码长度需要在 6 到 32 之间",
  passwords_do_not_match: "密码不一致",
  username_unavailable_message: "用户名不可用。",
  email_unavailable_message: "邮箱不可用。",
  invalid_password_message: "密码无效。",
  passwords_do_not_match_message: "两次输入的密码不一致。",
  response_already_loggedin: "你已经登陆过。",
  response_duplicate_username: "用户名已被使用。",
  response_duplicate_email: "邮箱已被使用。",
  success: "注册成功，{username}！"
};

    ...
    for (let i = 0; i < graph.nodes().length - 1; i++) {
        ...
        if (mat[preNode][insNode] < mat[postNode][insNode]) {
            ...
            for (let edge of graph.edges()) {
                if (edge.source == preNode && edge.target == insNode || edge.source == insNode && edge.target == curNode) {
                    edge.datum.chosen = 1;
                }
            ...
            for (let edge of graph.edges()) {
                if (edge.source == curNode && edge.target == insNode || edge.source == insNode && edge.target == postNode) {
                    edge.datum.chosen = 1;
                }

...
class HungarianDFS extends GraphAlgorithm {
  ...
  * dfs(pos: number) {
    let sn = this.nodes[pos], sd = sn.datum;
    for (let e of this.adjlist.adjacentEdges(sn.id)) {
      let tn = this.nodes[e.target], td = tn.datum;
      if (!td.visit) {
        td.visit = true;
  ...
  *run(graph: Graph): Generator<Step> {
    ...
    this.edges.map(edge => this.nodes[edge.source].datum.side === "left" ? edge : {
        source: edge.target,
        target: edge.source,
        datum: edge.datum
      }
    ...
    for (let leftnode of left) {
      ...
      if (!(yield* this.dfs(leftnode.id)))
        this.nodes[leftnode.id].datum.tag = 2;
